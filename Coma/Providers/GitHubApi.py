...
class GitHub(Provider):
    ...
    def GetData(component: Component) -> Optional[GitHubData]:
        ...
        path = path.format(component.name, component.author)
        ...
        data = GitHubData(clone_url, website, owner, 
                          contributors, num_contributors)
        
    # The contributor list INCLUDES the owner
    @staticmethod
    def GetContributors(url: str, max=None) -> tuple[List[Author], int]:
        contrib_result = requests.get()
        if not contrib_result.ok: return None
        contrib_content = contrib_result.json()
        
        num_contributors = len(contrib_content)
        contributors = []
        for i, contributor in enumerate(contrib_content):
    ...
    @staticmethod
    def GetReleases(component: Component) -> List[Release]:
    ...
    @staticmethod
    def GetBranches(component: Component) -> List[Branch]:
